...
class NaukriPlatform(BasePlatform):
    ...
    def _build_search_url(
        self, keyword: str, location: str, time_filter: Optional[str] = None
    ) -> str:
        """
        Constructs Naukri search URL with time filters:
        - 24h -> days=1 / q_days=1
        - 7d  -> days=7 / q_days=7
    ...
    def run(
        ...
        """Executes Naukri direct apply loop across keywords, locations, and time filter."""
        ...
        try:
            ...
            for keyword in keywords:
                ...
                for location in locations:
                    ...
                    logger.step(f"Searching Naukri jobs for '{keyword}' in '{location}' [Filter: {time_filter}]...")
                    search_url = self._build_search_url(keyword, location, time_filter=time_filter)
                    page.goto(search_url, wait_until="domcontentloaded", timeout=30000)
                    time.sleep(3.0)
                    ...
                    # Sort candidates so Early/Top Match AND high CV skill matches are applied to first
                    ordered_jobs = sorted(
                        job_cards_data,
                        key=lambda j: (j["is_priority"], j["skill_match_count"]),
                        reverse=True,
                    )
                    ...
                    logger.info(
                        ...
                        f"({len(high_match_jobs)} with high CV skills match, {len(priority_jobs)} Early/Top Match)."
                    )
