...
class LinkedInPlatform(BasePlatform):
    ...
    def _build_search_url(
        ...
        if easy_apply_only:
            params.append("f_AL=true")  # Restrict to Easy Apply only
        ...
        if start > 0 or page_no is not None:
    ...
    def _process_search_page(
        ...
        """Processes all eligible job cards on the current LinkedIn search results page."""
        BrowserManager.smooth_scroll(page, scrolls=3, distance=500)
        ...
        for c in raw_cards:
            try:
                ...
                card_comp = comp_elem.inner_text(timeout=1000).strip() if comp_elem.count() > 0 else ""
        ...
        for job_info in ordered_jobs:
            ...
            try:
                ...
                if not opened:
                    # Safe non-destructive fallback: JS click directly on element without losing search page
                    try:
                        if card_loc.count() > 0:
                ...
                if not opened:
                    logger.info(f"Could not interact with card #{job_id} in search view. Skipping card to maintain search context.")
                    continue

                time.sleep(2.0)

                # Extract job title & company from top card or details pane
                title_loc = page.locator(
                    ".job-details-jobs-unified-top-card__job-title, h1.t-24, h1.top-card-layout__title, .jobs-unified-top-card__job-title, div.jobs-details__main-content h1, h1"
                )
                company_loc = page.locator(
                    ".job-details-jobs-unified-top-card__company-name, .jobs-unified-top-card__company-name, div[data-view-name='job-details-top-card'] a[href*='/company/'], .topcard__flavor--black-link, a.topcard__org-name-link"
                )

                job_title = title_loc.first.inner_text().strip() if title_loc.count() > 0 else ""
                if not job_title or job_title == "Unknown Title":
                    # Fallback to verified title extracted during card discovery
                    job_title = job_info.get("title") or "Unknown Title"

                company_name = company_loc.first.inner_text().strip() if company_loc.count() > 0 else ""
                if not company_name or company_name == "Unknown Company":
                    # Fallback to verified company extracted during card discovery
                    company_name = job_info.get("company") or "Unknown Company"
                ...
                if not is_easy_apply:
                    easy_apply_only = self.linkedin_cfg.get("easy_apply_only", True)
                    if easy_apply_only:
                        logger.info(f"Skipping external application on LinkedIn: '{job_title}' at '{company_name}' ('easy_apply_only' is enabled)")
                        continue

                    ext_enabled = self.config.get("external_apply", {}).get("enabled", False)
                    if not ext_enabled:
                        logger.info(f"Skipping external company application for: '{job_title}' at '{company_name}'")
                        continue
                    ...
                    if ext_page:
                        ...
                        # Ensure search page context wasn't navigated away
                        if "jobs/search" not in (page.url or ""):
                            try:
    ...
    def run(
        ...
        """Executes LinkedIn Easy Apply loop across keywords, locations, deep pages, and time filter."""
            ...
            for keyword in keywords:
                ...
                for location in locations:
                    ...
                    for page_idx in range(max_pages):
                        ...
                        start = page_idx * 25
                        page_num_str = f" (Page {page_idx + 1})" if page_idx > 0 else ""
                        logger.step(f"Searching LinkedIn jobs{page_num_str} for '{keyword}' in '{location}' [Filter: {time_filter}]...")
                        search_url = self._build_search_url(keyword, location, time_filter=time_filter, start=start)
                        page.goto(search_url, wait_until="domcontentloaded", timeout=30000)
                        time.sleep(3.0)
                        self._ensure_workplace_types(page)
